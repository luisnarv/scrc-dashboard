...
import { ButtonMenuOperativo, SegmentedControl } from '../components/Buttons';
import { useTheme } from '../components/ThemeProvider';
import type { RawRecord } from '../components/utils/types';
...
export default function InformesPage() {
  const { raw, mesList, loading: dashLoading, error: dashError } = useDashboard();
...
=======
  // Metadata compartida para selectores (zonas y meses)
  const [meta, setMeta] = useState<{ zonas: string[]; meses: string[] }>({ zonas: [], meses: [] });
  ...
  const [prodRows, setProdRows] = useState<TecnicoProduccion[]>([]);
  const [prodTotales, setProdTotales] = useState<{ ordenes: number; produccion: number; meta: number; faltante: number }>({
    ordenes: 0,
    produccion: 0,
    meta: 0,
    faltante: 0,
...
>>>>>>> Stashed changes
  ...
  const [tipo, setTipo] = useState<'operativas' | 'disponibles' | ''>('');
...
<<<<<<< Updated upstream
=======
  // Carga de metadatos para listas desplegables
>>>>>>> Stashed changes
  useEffect(() => {
    fetch('/api/informes/digitacion?meta=1')
      ...
      .then(d => {
        ...
        setMeta({ zonas, meses });
        if (meses.length) {
          setMes(prev => prev || meses[0]);
          setProdMes(prev => prev || meses[0]);
        }
      })
      .catch(() => {
        if (mesList.length) {
          setMes(prev => prev || mesList[0]);
          setProdMes(prev => prev || mesList[0]);
        }
      });
  }, [mesList]);
...
=======
  ...
  /* ─── Generación Informe de Producción ─── */
  const ejecutarCalculoProd = (
    mode: 'tecnico' | 'operativa',
    selMes: string,
    selFecha: string,
    selZona: string,
    selTipo: 'operativas' | 'disponibles' | ''
  ) => {
    if (!selMes) {
      setProdError('Selecciona un mes.');
      return;
    }
    setProdLoading(true);
    setProdError(null);
    ...
    try {
      const allRecords: RawRecord[] = raw?.raw || [];
      const filtered = allRecords.filter(r => {
        // Mes
        if (selMes && String(r.Fecha || '').slice(0, 7) !== selMes) return false;
        // Día exacto
        if (selFecha && String(r.Fecha || '').slice(0, 10) !== selFecha) return false;
        // Zona
        if (selZona) {
          const zUpper = selZona.toUpperCase().trim();
          const rz = String(r.Zona || r._Zona || r._ZonaDet || r.zona || r.Zona_Detalle || '').toUpperCase().trim();
          if (rz !== zUpper && !rz.includes(zUpper)) return false;
        }
        // Tipo de Brigada
        if (selTipo) {
          const brig = String(r.Tipo_Brigada_Operaciones || r.Tipo_Brigada_Mes || r.Tipo_Cuadrilla || r.brigada_homologada || '');
          const isDisp = isDisponibleType(brig);
          if (selTipo === 'disponibles' && !isDisp) return false;
          if (selTipo === 'operativas' && isDisp) return false;
        }
        return true;
      });
      ...
      filtered.forEach(r => {
        ...
        if (mode === 'tecnico' && r.Nombre && String(r.Nombre).trim()) {
          acc.nombre = String(r.Nombre);
        }
      });

      const calculatedRows: TecnicoProduccion[] = Array.from(agg.entries())
        .map(([key, acc]) => ({
          cedula: mode === 'tecnico' ? key : '',
          nombre: acc.nombre,
          tipoBrigada: mode === 'tecnico' ? acc.tipoBrigada : key,
          ordenes: acc.ordenes,
          produccion: acc.produccion,
          meta: acc.meta,
          faltante: Math.max(0, acc.meta - acc.produccion),
      ...
      const calculatedTotales = calculatedRows.reduce(
        (t, r) => ({
          ordenes: t.ordenes + r.ordenes,
          produccion: t.produccion + r.produccion,
          meta: t.meta + r.meta,
      ...
      setProdRows(calculatedRows);
      setProdTotales(calculatedTotales);
    } catch (e) {
      setProdError(String(e instanceof Error ? e.message : e));
      setProdRows([]);
      setProdTotales({ ordenes: 0, produccion: 0, meta: 0, faltante: 0 });
    } finally {
  ...
  const generarProd = () => {
    ejecutarCalculoProd(viewMode, prodMes, prodFecha, prodZona, prodTipo);
  };

  const cambiarViewMode = (nuevoModo: 'tecnico' | 'operativa') => {
    setViewMode(nuevoModo);
    if (prodGenerado) {
      ejecutarCalculoProd(nuevoModo, prodMes, prodFecha, prodZona, prodTipo);
    }
  };

  /* ─── Descarga Genérica ─── */
  const descargar = (contenido: BlobPart, mime: string, ext: string, filename: string) => {
  ...
  /* ─── Exportación Informe de Producción ─── */
  const nombreArchivoProd = () =>
    `informe_produccion_${prodMes}${prodFecha ? '_' + prodFecha : ''}${prodZona ? '_' + prodZona : ''}_${viewMode}`;

  const filasExportProd = () => {
    if (!prodRows.length) return [];
    const result: Record<string, string | number>[] = prodRows.map(r => {
      ...
      o['Cumplimiento (%)'] = `${pctCumplimiento(r.produccion, r.meta).toFixed(1)}%`;
      return o;
    });
    ...
    if (viewMode === 'tecnico') {
      totalRow['Técnico'] = 'TOTAL GENERAL';
      totalRow['Tipo Operativa'] = '';
    } else {
      totalRow['Operativa'] = 'TOTAL GENERAL';
    }
    totalRow['Órdenes'] = prodTotales.ordenes;
    totalRow['Producción ($)'] = prodTotales.produccion;
    totalRow['Meta del Día ($)'] = prodTotales.meta;
    totalRow['Faltante ($)'] = prodTotales.faltante;
    totalRow['Cumplimiento (%)'] = `${pctCumplimiento(prodTotales.produccion, prodTotales.meta).toFixed(1)}%`;
  ...
  /* ─── Generación Informe de Digitación ─── */
>>>>>>> Stashed changes
  const generar = async () => {
...
=======
  const totalesDigitacion = useMemo(() => {
>>>>>>> Stashed changes
...
<<<<<<< Updated upstream
  // ── Exportación ────────────────────────────────────────────────────────────
=======
  /* ─── Exportación Informe de Digitación ─── */
>>>>>>> Stashed changes
  const nombreArchivo = () =>
...
=======
  const exportCSV = () => {
    const csv = Papa.unparse(filasExport(), { delimiter: ';' });
    descargar('﻿' + csv, 'text/csv;charset=utf-8;', 'csv', nombreArchivo());
  };
  ...
  const exportExcel = () => {
    ...
    const tbody = data.map(r => '<tr>' + cols.map(c => `<td>${esc(r[c])}</td>`).join('') + '</tr>').join('');
    const html = `<html><head><meta charset="utf-8"></head><body><table border="1">${thead}${tbody}</table></body></html>`;
    descargar(html, 'application/vnd.ms-excel', 'xlsx', nombreArchivo());
  };
...
>>>>>>> Stashed changes
  ...
  const btn = (bg: string): React.CSSProperties => ({ padding: '8px 16px', borderRadius: 8, border: 'none', background: bg, color: '#fff', fontSize: 13, fontWeight: 700, cursor: 'pointer', transition: 'opacity .15s' });
...
<<<<<<< Updated upstream
=======
  if (dashLoading) return <div className="loading-wrap"><div className="spinner" /><span>Cargando datos…</span></div>;
  if (dashError) return <div className="status err">{dashError}</div>;
...
=======
      {/* ═══════════════════════════════════════════════════════════════ */}
      {/* ═══ INFORME DE PRODUCCIÓN (COLAPSABLE CON FILTROS PROPIOS) ═══ */}
      {/* ═══════════════════════════════════════════════════════════════ */}
      <button
        onClick={() => setProdAbierto(v => !v)}
        style={{
      ...
      >
        ...
        <span style={{ display: 'flex', flexDirection: 'column' }}>
          ...
          <span style={{ fontSize: 12, color: MUT }}>Producción valorizada, metas diarias y faltante por técnico u operativa</span>
        </span>
        <span style={{ marginLeft: 'auto', color: MUT, fontSize: 13 }}>{prodAbierto ? '▲' : '▼'}</span>
      </button>

      {/* Panel de Filtros y Acciones de Producción */}
      {prodAbierto && (
            ...
            <div>
              ...
              <SegmentedControl
                options={[
                  ...
                  { label: 'Por Operativa', value: 'operativa' },
                ]}
                value={viewMode}
                onChange={v => cambiarViewMode(v as 'tecnico' | 'operativa')}
      ...
      {prodAbierto && prodGenerado && !prodError && (
        <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 10, overflow: 'hidden' }}>
          <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--border)', fontSize: 13.5, fontWeight: 700, color: INK }}>
            Producción {prodMes}{prodFecha ? ` · ${prodFecha}` : ''}{prodZona ? ` · ${prodZona}` : ''}{prodTipo ? ` · ${prodTipo}` : ''} ({viewMode === 'tecnico' ? 'Por Técnico' : 'Por Operativa'})
            <span style={{ fontWeight: 500, color: MUT }}> — {prodRows.length} {viewMode === 'tecnico' ? 'técnicos' : 'operativas'}</span>
          </div>
          ...
          {prodRows.length === 0 && !prodLoading ? (
            <div style={{ ...card, textAlign: 'center', padding: '40px 20px', color: MUT }}>
              ...
              <div style={{ fontSize: 12.5, marginTop: 4 }}>
                Ajusta los filtros de fecha, zona o brigada para ver el informe de producción.
              </div>
            </div>
          ) : (
...
=======
                  value: fmtN(prodRows.length),
                  color: TEAL,
                },
                { label: 'Total Órdenes', value: fmtN(prodTotales.ordenes), color: INK },
                { label: 'Producción Total', value: fmtCOP(prodTotales.produccion), color: OK },
                { label: 'Meta Total', value: fmtCOP(prodTotales.meta), color: INK },
                { label: 'Faltante Total', value: fmtCOP(prodTotales.faltante), color: colorFaltante(prodTotales.faltante, prodTotales.meta) }]
>>>>>>> Stashed changes
                  .map((item, i) => (
                    <div key={i} style={{
                      padding: '16px 20px',
...
=======
                            ...
                            <td style={{ ...tdStyle, fontWeight: 600 }}>{row.tipoBrigada}</td>
>>>>>>> Stashed changes
                          )}
...
=======
                        TOTALES ({prodRows.length} {viewMode === 'tecnico' ? 'técnicos' : 'operativas'})
                      </td>
                      <td style={{ ...tdStyleRight, fontWeight: 800 }}>{fmtN(prodTotales.ordenes)}</td>
                      <td style={{ ...tdStyleRight, fontWeight: 800, color: OK }}>{fmtCOP(prodTotales.produccion)}</td>
                      <td style={{ ...tdStyleRight, fontWeight: 800 }}>{fmtCOP(prodTotales.meta)}</td>
                      <td style={{ ...tdStyleRight, fontWeight: 800, color: colorFaltante(prodTotales.faltante, prodTotales.meta) }}>
...
=======
                          color: pctCumplimiento(prodTotales.produccion, prodTotales.meta) >= 100 ? OK : pctCumplimiento(prodTotales.produccion, prodTotales.meta) >= 70 ? WARN : ERR,
                          background: (pctCumplimiento(prodTotales.produccion, prodTotales.meta) >= 100 ? OK : pctCumplimiento(prodTotales.produccion, prodTotales.meta) >= 70 ? WARN : ERR) + '18',
                        }}>
                          {pctCumplimiento(prodTotales.produccion, prodTotales.meta).toFixed(0)}%
                        </span>
>>>>>>> Stashed changes
                      </td>
                    </tr>
...
=======
      {/* ═══════════════════════════════════════════════════════════════ */}
      {/* ═══ INFORME DE DIGITACIÓN (COLAPSABLE) ═════════════════════════ */}
      {/* ═══════════════════════════════════════════════════════════════ */}
>>>>>>> Stashed changes
      <button
...
=======
                      {COLS.map(c => <td key={c.key} style={{ ...td, fontWeight: 800, color: INK }}>{totalesDigitacion[c.key]}</td>)}
>>>>>>> Stashed changes
                    </tr>
                  </tfoot>
                )}
